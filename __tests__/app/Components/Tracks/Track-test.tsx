...
describe("group tesing goes here", () => {
    ...
    //     const { getByTestId } = render(<PlaylistsTracksCard setSong={'ABC'} playlistRef={playlistReference} name='NOOR' model='123' img='img' item='item' />);
    //     fireEvent.press(getByTestId('setASongs'));
    // });
    test('setting songs', () => {
        const playlistReference = { current: { snapToIndex: 0 } };
        const { getByTestId } = render(<PlaylistsTracksCard setSong={'item'} />);
